...
class EmtSimu():
    ...
    def updateIibr(self, pfd, dyd, ini):
        ...
        mode = getattr(self, "control_mode", "GFL")

        # Always run the original kernel
        numba_updateIibr(
            self.Igi,
            self.Iibr,
            pfd.ibr_bus,
        ...
        if mode == "GFM":
            # Override by adding a GFM injection at gen bus 1 (or first gen)
            if len(pfd.gen_bus) > 0:
                g_bus = int(pfd.gen_bus[0])
                idx_list = np.where(pfd.bus_num == g_bus)[0]
                if idx_list.size > 0:
                    bus_idx = int(idx_list[0])

                    if not hasattr(self, "_gfm_debug_printed"):
                        print("GFM overriding at gen bus:", g_bus, "bus_idx:", bus_idx)
                        self._gfm_debug_printed = True

                    Vb = vtemp[bus_idx]
                    V_nom = 1.0
                    k_v = 1.0   # can tune later

                    V_mag = np.abs(Vb)
                    if V_mag > 1e-6:
                        I_gfm = k_v * (V_nom - V_mag) * (Vb / V_mag)
                    else:
                        I_gfm = 0.0 + 0.0j

                    # Make sure node_Ihis exists and has correct length
                    if not hasattr(self, "node_Ihis") or self.node_Ihis.size == 0:
                        # initialize once with zeros of bus length
                        self.node_Ihis = np.zeros_like(self.Vsol, dtype=complex)

                    # Add GFM current injection at that bus
                    self.node_Ihis[bus_idx] += I_gfm
